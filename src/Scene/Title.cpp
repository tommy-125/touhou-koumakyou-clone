...
Title::Title() : m_MainMenuBlackMask(0.5f, 0.0f), m_LeaveMainMenuBlackMask(2.0f, 0.0f) {
    ...
    std::vector<Anm::LoadedEntry> loaded = {
        ...
        {&Anm::TITLE01S,
         m_Anm.LoadAnm(Anm::TITLE01S.folder, Anm::TITLE01S.txt, Anm::TITLE01S.offset)},
...
void Title::Update() {
        ...
        case TitleState::MainMenu: {
            if (Util::Input::IsKeyDown(Util::Keycode::UP)) {
                ...
                if (static_cast<TitleMenuItem>(m_SelectedMenuItemIdx) ==
                    TitleMenuItem::ExtraStart) {  // skip ExtraStart
                    m_SelectedMenuItemIdx =
                        (m_SelectedMenuItemIdx - 1 + TITLE_MENU_COUNT) % TITLE_MENU_COUNT;
                }
    ...
    for (int i = 0; i < TITLE_MENU_COUNT; i++) {
        int selectedMenuSpriteIdx =
            m_UnselectedMenuItemVms[i]->spriteIdx - Anm::TITLE01.offset +
            Anm::TITLE01S.offset;  // calculate selected menu item sprite index

        if (i == m_SelectedMenuItemIdx) {
            if (m_Anm.sprites[selectedMenuSpriteIdx].image) {
                m_UnselectedMenuItemVms[i]->obj->SetDrawable(
                    m_Anm.sprites[selectedMenuSpriteIdx]
                        .image);  // show the currently selected menu item
