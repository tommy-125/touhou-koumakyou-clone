...
void RunStarOfDavid(Enemy& enemy, EnemySubCtx& ctx, int t) {
    if (!BeginRemiliaSpellAt(enemy, ctx, t, "Heaven's Punishment \"Star of David\"", 4,
                             2400, SUB_REMILIA_NONSPELL_2, {192.0f, 112.0f})) {
        return;
    }
...
void RunVlad(Enemy& enemy, EnemySubCtx& ctx, int t) {
    const bool active = BeginRemiliaSpellAt(enemy, ctx, t, "Curse \"Curse of Vlad Tepes\"", 2,
                                            2400, SUB_REMILIA_NONSPELL_4, {192.0f, 144.0f});
    if (t == 0 || t == 120) {
