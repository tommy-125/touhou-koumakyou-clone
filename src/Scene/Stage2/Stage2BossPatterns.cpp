    ...
    switch (enemy.m_SubId) {
        ...
        case SUB_CIRNO_NONSPELL_ATTACK_A: {
            ...
            if (t >= 41 && t < 41 + 3 * 70) {
                ...
                if (local <= 10 && local % 2 == 0) {
                    ...
                    ctx.bullets.SpawnFanAimed(ScriptUtil::ShootPos(enemy, CIRNO_SHOOT_OFFSET),
                                              ctx.playerPos, EBulletType::Shard, EBulletColor::Blue,
                                              fanIdx + 1, 5.0f - static_cast<float>(fanIdx) * 0.5f,
        ...
        case SUB_CIRNO_ICICLE_FALL: {
            ...
            if (t >= 120) {
                ...
                if (loopT < 297 && loopT % 27 == 0) {
                    ...
                    SpawnOneWayStackWithCurve(shootPos, ctx, EBulletType::Shard, EBulletColor::Blue,
                                              stacks, 6.5f, 0.5f, rightBase + jitter, rightCurve,
                                              true);
                    SpawnOneWayStackWithCurve(shootPos, ctx, EBulletType::Shard, EBulletColor::Blue,
                                              stacks, 6.5f, 0.5f, leftBase - jitter, leftCurve,
                                              true);
        ...
        case SUB_CIRNO_PREFREEZE_ATTACK_A: {
            ...
            if (t >= 0 && t < 160 && t % 20 == 0) {
                ...
                ctx.bullets.SpawnCircleAimed(ScriptUtil::ShootPos(enemy, CIRNO_SHOOT_OFFSET),
                                             ctx.playerPos, EBulletType::RingBall,
                                             EBulletColor::Blue, count, 3.0f);
            }
            if (t >= 10 && t < 170 && t % 20 == 10) {
                ctx.bullets.SpawnCircleAimed(ScriptUtil::ShootPos(enemy, CIRNO_SHOOT_OFFSET),
                                             ctx.playerPos, EBulletType::Pellet,
            ...
            if (t >= 120) {
                ...
                if (loopT >= 5 && loopT < 155 && (loopT - 5) % 5 == 0) {
                    ...
                    SpawnRandomVarianceCircle(ctx, ScriptUtil::ShootPos(enemy, CIRNO_SHOOT_OFFSET),
                                              EBulletType::RingBall, PerfectFreezeColor(colorIdx),
                                              count, 4.0f);
                }
                if (loopT == 175) {
