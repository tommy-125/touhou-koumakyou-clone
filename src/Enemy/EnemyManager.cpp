...
static constexpr int TIMELINE_SIZE =
...
Enemy *EnemyManager::SpawnEnemy(int subId, float x, float y, int life,
                                int score, bool mirrored) {
...
void EnemyManager::RunTimeline() {
    while (m_TimelineIdx < TIMELINE_SIZE &&
           STAGE1_TIMELINE[m_TimelineIdx].frame <= m_Frame) {
...
void EnemyManager::Update(const glm::vec2 &playerPos) {
    ...
    for (auto &enemy : m_Enemies) {
        ...
        if (enemy.m_Pos.x < -200 || enemy.m_Pos.x > 600 ||
            enemy.m_Pos.y < -200 || enemy.m_Pos.y > 700) {
            enemy.m_Alive = false;
            if (enemy.m_Vm.obj) {
                m_Renderer.RemoveChild(enemy.m_Vm.obj);
                enemy.m_Vm.obj = nullptr;
