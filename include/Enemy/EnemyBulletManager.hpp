...
class EnemyBulletManager {
   public:
    ...
    void SpawnCircle(glm::vec2 pos, EBulletType type, EBulletColor color, int count, float speed,
                     float baseAngle = 0.0f, bool useDecay = false, float acceleration = 0.0f,
                     int accelerationFrames = 0, bool rotateWithAngle = false,
                     glm::vec2 vectorAcceleration = {0.0f, 0.0f}, int vectorAccelerationFrames = 0,
                     int spawnMoveFrames = 0, float spawnMoveScale = 1.0f, BulletCurve curve = {},
                     bool bounceTopAndSides = false, int bounceMax = 0, float bounceSpeed = -1.0f,
    ...
    int  DespawnBulletsForBonus(ItemManager* items, int maxBonusScore, bool awardPointItems);
